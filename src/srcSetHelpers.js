...
export async function getSrcSet({ srcDir, fileName, width, resolutions, staticPrefix }) {
  ...
  return resolutions
    .map((resolution) => {
      ...
      return {
        src: `${staticPrefix || ''}${base}-${intrinsicWidth}.${extension}`,
        intrinsicWidth,
        intrinsicHeight,
        aspectRatio,
        resolution,
...
export function getSrcSets({ srcDir, fileName, widths, resolutions, staticPrefix }) {
